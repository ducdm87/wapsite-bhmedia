...
class APPSession extends APPObject
{
	/**
	 * internal state
	 *
	...
	*/
	...
	{
            if(isset($options['table'])) $this->_tbl = $options['table'];
	...
	function & getInstance($handler, $options)
	{
		...
		if (!is_object($instance)) {
			$instance = new APPSession($handler, $options);
		}
...
//				return false;
	...
	function readSession($id)
	{
		...
		$query = "SELECT * FROM ".$this->_tbl." WHERE session_id = ". $db->quote($id);
	 ...
	 * @return unknown
	 */
	function writeSession($id, $data = false)
	{
		global $config, $db,$user;
		
		$username 	= is_object($user)?$user["username"]:"";
		$usertype 	= is_object($user)?$user["usertype"]:"";
		$userid 	= is_object($user)?$user["id"]:"";
		$gid	 	= is_object($user)?$user["gid"]:"";
		if(!$data)
		{
			$vars = $_SESSION;
